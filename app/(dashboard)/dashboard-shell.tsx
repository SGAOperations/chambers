...
export default function DashboardShell({
  ...
  // Resolved on the server and shipped with the document -- see
  ...
  // alerts. The sidebar badge and the notification bell are both decorations on
  // an already-rendered shell, so this is the only network the shell still does
  // after hydration -- and nothing waits on it.
  ...
  useEffect(() => {
    // Wrapped rather than called bare so the setState calls inside fetchDashboard
    // are unambiguously after an await -- react-hooks/set-state-in-effect flags a
    // direct call, since it cannot see that the first statement in the body is a
    // fetch.
    const load = async () => {
      ...
      <div className="flex h-screen">
        <nav className={`fixed inset-y-0 left-0 z-40 w-56 bg-[#0a1628] flex flex-col flex-shrink-0 transition-transform duration-300 md:relative md:translate-x-0 md:inset-auto md:z-30 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <div className="px-5 py-5 border-b border-white/10">
            ...
            <p className="text-slate-600 text-xs mt-1">v1.16.0</p>
            {userName && (
        ...
        <main className="flex-1 bg-gradient-to-br from-[#112244] via-[#0a1628] to-[#060e1a] p-8 pt-20 md:pt-8 overflow-y-auto overflow-x-hidden">
          {/* No auth gate here any more: the server layout above resolved the
              session before this component was ever sent, so `children` is safe
              to render immediately -- there is nothing left to wait for. */}
