...
export async function PATCH(request: Request) {
  ...
  const [{ data: prevBooking }, { data: prevTabling }, { data: prevSessions }] = await Promise.all([
    ...
    adminSupabase
      .from('tabling_sessions')
      .select('location, session_date, start_time, end_time, status, reservation_code')
  ...
  type TablingValues = {
    location: string; date: string; start: string; end: string; status: string; code: string | null
  }
  const tablingValues = (r: { location: string; session_date: string; start_time: string; end_time: string; status: string; reservation_code: string | null }): TablingValues => ({
    location: r.location,
    date: r.session_date,
    start: r.start_time,
    end: r.end_time,
